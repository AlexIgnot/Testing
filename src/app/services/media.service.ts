...
getProxySize(): Observable<number[]> {
	...
	return this.http.get<{data: number[]}>(this.apiURLProxySize).pipe(	map(response => response.data)
	);
}

getDevHours(): Observable<number[]> {

	return this.http.get<{data: number[]}>(this.apiURLDevHours).pipe(map(response => response.data)	);
